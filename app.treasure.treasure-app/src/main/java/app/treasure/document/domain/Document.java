package app.treasure.document.domain;
...
public class Document extends PanacheEntity
{
	 ...
	 */
	public void addTag(app.treasure.shared.domain.Tag tag, TagSource source)
	{
		// Check if this tag already exists on the document (by ID or name)
		boolean exists = documentTags.stream()
			.anyMatch(dt -> {
				app.treasure.shared.domain.Tag existingTag = dt.getTag();
